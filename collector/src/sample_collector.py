...
from utility import c_data_handler_wrapper, HandlerResult
...
def data_handler(data: bytearray):
    ...
    status, results = c_data_handler_wrapper(data)
    if status == HandlerResult.RESULT_DATA_PROCESSED:
        data_received_tmp.extend(results)
    elif status == HandlerResult.RESULT_STOP_CMD_RECEIVED:
    ...
    elif status == HandlerResult.RESULT_TIME_SYNC_PROCESSED:
        print("Time synchronization processed successfully.")
    elif status < 0:
